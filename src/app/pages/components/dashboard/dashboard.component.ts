...
@Input() isCollapsed = false;
  ...
  get userImageUrl(): string { return 'https://via.placeholder.com/80/007bff/ffffff?text=SP'; }

  // Método para abrir el diálogo de subida de evidencia
  openUploadEvidenceDialog(): void {
    const dialogRef = this.dialog.open(UploadEvidenceDialogComponent, {
      width: '600px',
      disableClose: true
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log('El diálogo de subida se cerró');
      if (result === true) {
        // Opcional: Mostrar mensaje de éxito general o recargar algo si es necesario
        // Por ejemplo, si la lista de notificaciones se actualiza tras subir fotos
        // this.authService.refreshNotifications(); // (Método hipotético)
      }
    });
  }
}
